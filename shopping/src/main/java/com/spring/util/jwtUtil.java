...
public class jwtUtil {
	...
	public String createToken(String session_email, String session_pass, String session_type) {
		...
		String br = Jwts.builder().setIssuer(ISSUER).setClaims(map).setSubject(SUBJECT).signWith(signAlg, Key)
				.compact();// Mi creo un oggetto che crea jwt string(.builder()) , l'emittente =
							// (setIusser) , il playload(setclaim()),l'utente "per cui viene creato"
							// (setSubject(), e firmo con key e alg
		System.out.println("Sto stampando il token :  " + br);
		return br;
	}
